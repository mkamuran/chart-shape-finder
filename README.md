...
ブラウザからそのまま操作できます。チャート形状を手描きし、基準日・期間・出来高条件を指定すると、条件に近い値動きをした銘柄をランキング表示します。

Render の無料枠で公開しているため、初回アクセスや初回検索は少し時間がかかることがあります。

![Chart Shape Finder screenshot](docs/screenshot.png)

## Example Search Result

実際に検索した結果画面です。左側に類似度ランキング、銘柄名、業種、終値、平均出来高、重ね合わせチャートを表示し、右側には検索条件を残しています。

金融領域の人に見せる前提で、結果だけでなく「どの市場・期間・基準日・条件で出した結果なのか」を同じ画面内で確認できるようにしています。

![Search results with conditions](docs/search-results.png)

## Financial Viewpoint

- チャート形状から銘柄候補を探す、探索型のスクリーニングUIです。
- 終値系列を正規化して比較するため、株価水準ではなく値動きの形に注目できます。
- 基準日、比較期間、出来高条件を明示し、結果の前提を確認しやすくしています。
- 売買推奨ではなく、チャートパターン探索のデモとして実装しています。
